...
static bool isDate(const std::string &date) {
  ...
  if (t == -1)
    return false;
  // std::cout << "year: " << year - 1900 << " month: " << month - 1
  //           << " day: " << day << std::endl;
  // std::cout << "tm.tm_year: " << tm.tm_year << " tm.tm_mon: " << tm.tm_mon
  //           << " tm.tm_mday: " << tm.tm_mday << std::endl;
  return tm.tm_mday == day && tm.tm_mon == month - 1 &&
         tm.tm_year == year - 1900 && tm.tm_wday != 0 && tm.tm_wday != 6 &&
         tm.tm_yday != 0;
}
