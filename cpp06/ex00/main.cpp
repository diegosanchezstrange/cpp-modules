...
{
	std::string printable = str[0] == 'n' ? "nan" : std::string("inf").insert(0, 1, str[0]);
...
int	is_number(std::string str)
{
	...
	if (n == std::string::npos)
		return (1);
	else if (str[n] != '.' || str.find_first_not_of("123456789", n + 1) != std::string::npos)
		return (0);
	return (1);
}
...
int	main(int argc, char **argv)
{
	...
	std::cout << "char: " << (std::isprint(resInt) ? str : "Not displayable.") << std::endl;
	std::cout << "int: " << resInt << std::endl;
	std::cout << "float: " << static_cast<float>(res) << (res - atoi(str.c_str()) > 0 ? "f" : ".0f") << std::endl;
	std::cout << "double: " << res << (res - atoi(str.c_str()) > 0 ? "" : ".0") << std::endl;
